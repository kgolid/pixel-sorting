...
let src_resolution = 1;

let expansion_candidates = 2;
let selection_size = 800;
let reversed = true;
...
let sketch = function(p) {
  ...
  p.preload = function() {
    img = p.loadImage('img2.jpg');
    src = p.loadImage('space.jpg');
  };
  ...
  p.setup = function() {
    ...
    originalpixels = newArray(ny).map((_, j) =>
      newArray(nx).map((_, i) => {
        var loc = (i + j * src.width) * 4 * src_resolution;
        return rgbColor
          ? [src.pixels[loc + 0], src.pixels[loc + 1], src.pixels[loc + 2]]
          : RGBtoHSV(
              src.pixels[loc + 0],
              src.pixels[loc + 1],
              src.pixels[loc + 2]
    ...
    imgpixels = newArray(ny).map((_, j) =>
      newArray(nx).map((_, i) => {
        ...
        return rgbColor
          ? [img.pixels[loc + 0], img.pixels[loc + 1], img.pixels[loc + 2]]
          : RGBtoHSV(
              img.pixels[loc + 0],
              img.pixels[loc + 1],
              img.pixels[loc + 2]
...
function getNearest(q, rs, reversed) {
  const reverse = Math.random() > 0.96 ? !reversed : reversed;
