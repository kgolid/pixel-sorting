...
let sketch = function(p) {
  ...
  p.preload = function() {
    src = p.loadImage('monet.jpg');
    img = p.loadImage('starry-night.jpg');
  };

  p.setup = function() {
    nx = Math.floor(src.width / src_resolution);
    ny = Math.floor(src.height / src_resolution);
    ...
    originalpixels = newArray(ny).map((_, j) =>
      newArray(nx).map((_, i) => {
        ...
        return [src.pixels[loc + 0], src.pixels[loc + 1], src.pixels[loc + 2]];
      })
    );
