...
let sketch = function(p) {
  ...
  p.setup = function() {
    ...
    drawPixel([...centre], start);
    candidates = expandNeighboorhood([centre], centre);
  ...
  function placePixels(n) {
    for (i = 0; i < n; i++) {
      ...
      candidates = expandNeighboorhood(candidates, resultPos);
      ...
      //imgpixels[resultPos[0]][resultPos[1]][3] = pixel_to_swap[3];
      //imgpixels[resultPos[0]][resultPos[1]][4] = pixel_to_swap[4];

      result[resultPos[0]][resultPos[1]] = pixel_to_swap;

      drawPixel([...resultPos], pixel_to_swap);
      //drawPixel([pixel_to_swap[3], pixel_to_swap[4]], [255, 255, 255]);
      pixels_placed++;
    }
  }
...
function getAdjacentIndices(q, include_diagonals) {
  ...
  if (include_diagonals) {
    if (q[0] < nx - 1) {
      if (q[1] < nx - 1) indices.push([q[0] + 1, q[1] + 1]);
...
function expandNeighboorhood(neighborhood, next) {
  ...
  //console.log(neighborhood[0], expansion);
  return union(neighborhood, expansion); // return union
}

function getSurroundingColor(q) {
  const adj = getAdjacentIndices(q, true).filter(
    pos => result[pos[0]][pos[1]] !== null
  );
  return meanColor(adj.map(ind => result[ind[0]][ind[1]]));
}

function meanColor(arr) {
  //console.log('nonempty adj', arr);
